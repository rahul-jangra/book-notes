...
- As long as *happens-before* guarantee is not violated, JMM is allowed to reorder the operations. JMM is specified in form of happens-before rules
...
##### 16.2 Publication Safety and Initialization Safety
- Suppose a thread constructs an object and stores its reference in a class variable. It is possible that other thread might not see updated value of this reference due to previous thread cache (visibility issue). But more dangerous is that other thread might see an updated reference but partially constructed object (Inside the object its fields allocation might not have completed yet due to reordering). - This is called unsafe publishing (another thread could see an up-to-date value for the object reference but *out-of-date values for some or all of that object’s state- a partially constructed object*.)
- For objects with final fields, initialization safety prohibits reordering any part of construction with the initial load of a reference to that object. All writes to final fields made by the constructor, as well as to any variables reachable through those fields, become “frozen” when the constructor completes, and any thread that obtains a reference to that object is guaranteed to see a value that is at least as up
